...
def get_url_problems_count(job_mgr: JobManagerAbs, get_all: bool = False) -> int:
    if not get_all:
        return sum(job.last_failed == True for job in job_mgr.get_url_jobs())
    else:
...
def get_burl_problems_count(job_mgr: JobManagerAbs, get_all: bool = False) -> int:
    if not get_all:
        return sum(job.last_failed == True for job in job_mgr.get_backbone_jobs())
    else:
...
def get_sip_problems_count(job_mgr: JobManagerAbs, get_all: bool = False) -> int:
    if not get_all:
        return sum(job.last_failed == True for job in job_mgr.get_sip_jobs())
    else:
...
def get_socket_problems_count(job_mgr: JobManagerAbs, get_all: bool = False) -> int:
    if not get_all:
        return sum(job.last_failed == True for job in job_mgr.get_socket_jobs())
    else:
...
def get_mongo_problems_count(job_mgr: JobManagerAbs, get_all: bool = False) -> int:
    if not get_all:
        return sum(job.last_failed == True for job in job_mgr.get_mongo_jobs())
    else:
...
def get_occurred_problems_count(job_mgr: JobManagerAbs) -> int:
